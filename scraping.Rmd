...
# Old code to scrape the github organization info
...
URL_OCS <- c("https://github.com/orgs/opencasestudies/repositories?type=all")
webpage <- URL_OCS %>%
  xml2::read_html() 
webpage


URL_OCS2 <- c("https://github.com/orgs/opencasestudies/repositories?type=all&page=2")
...
webpage_element <- webpage %>%
  rvest::html_elements(xpath= '//*[contains(concat( " ", @class, " " ), concat( " ", "klnPdE", " " ))]')
webpage_element2 <- webpage2 %>%
  rvest::html_elements(xpath= '//*[contains(concat( " ", @class, " " ), concat( " ", "klnPdE", " " ))]')

text_ocs <- webpage_element %>%
  rvest::html_text()
text_ocs2 <- webpage_element2 %>%
  rvest::html_text()
...
repodf2 <- data.frame(name = unlist(strsplit(text_ocs2, "Updated"))) 
repodf <-full_join(repodf, repodf2)
```


```{R}
 repodf <- repodf %>% mutate(is_case_study = str_detect(name, "ocs-bp-")) %>% filter(is_case_study == "TRUE") %>%
  ...
  separate(reponame,sep = "ocs", into = c("date", "name")) %>% 
  mutate(name = paste0("ocs", name))
...
```{r}
repo_non_bp <- repodf %>% filter(!str_detect(name, "-bp"))
...
OCS_info <- repodf %>%
  tibble(
    repo_url =  paste0("https://github.com/", name), 
    case_study_url =  paste0("https://www.opencasestudies.org/", name))
```
...
save(OCS_info, file = here::here( "ocs_info_int_with_repo_new.rda"))
```
